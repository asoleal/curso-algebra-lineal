...
# 2.1 Determinantes: El factor de escala del espacio {.unnumbered}
...
Sea $A$ una matriz de $n \times n$. El **menor** $M_{ij}$ es la matriz de $(n-1) \times (n-1)$ que resulta de eliminar la fila $i$ y la columna $j$ de $A$. El **cofactor** $C_{ij}$ se define como:

$$ C_{ij} = (-1)^{i+j} \det(M_{ij}) $$

El signo $(-1)^{i+j}$ crea un tablero de ajedrez de signos positivos y negativos.
:::

Aquí tienes el ejemplo de un cofactor para una matriz de $3 \times 3$, manteniendo exactamente el mismo formato y estilo:
...
Para calcular el cofactor $C_{12}$, primero eliminamos la fila $1$ y la columna $2$ para obtener el menor $M_{12}$:
...
Para calcular el cofactor $C_{12}$, eliminamos (tachamos) la fila $1$ y la columna $2$, obteniendo el menor $M_{12}$:
...
## 2.1.3 Ejercicios de mecanización
